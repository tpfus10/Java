...
public class Test01_1 {
	public static void main(String[] args) {
		...
		arr[0] = s;
		arr[1] = i1;
		//int len = arr[0].length();//Object에 length()가 없다->arr[0]은 Object로 선언되었음->Object에는 length가 없음
		int len = ((String) arr[0]).length();

		ArrayList lst = new ArrayList();//모든 타입의 객체들을 저장할 수 있다 그러나 비추다! 
										//- 이유는? 설정 안 하면 기본적으로 Object 객체로 간주하기 때문
		String s2 = "java";
		...
		System.out.println("list.size() = " + list.size());//list는 length가 아니라 size를 사용함 
		...
		// Arrays 클래스의 static <T> List<T> asList(T... a) *** 중요=뒤에서 써먹는다
		...
		// boolean containsAll(Collection<?> c) - 모든 collection의 하위 클래스는 다 된다는 의미
