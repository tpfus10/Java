...
public class Test06 {
	...
	public static void main(String[] args) {
		...
		HashMap<String, String> dic = new HashMap<>();//HashMap(key, value), 생성자

		for (int i = 0; i < word.length; i++) {
			dic.put(word[i], meaning[i]);//(key, value)
		}
		// dic.toString()은 AbstractMap에 저장됨
		System.out.println(dic);// dic.toString()생략됨, 출력 순서가 입력 순서와 다르다
		System.out.println(dic.size()); // 4

		// keySet(), values()는 collection 형태로 반환
		System.out.println(dic.keySet()); // toString메소드->[HEAVEN, BUMBLEBEE, WONDER, ALTHOUGH]
		System.out.println(dic.values()); // toString메소드->[천국, 꿀벌과에 속하는 호박벌, 호기심이 들다, 그럼에도 불구하고]
